...
class DeepseekOCRProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self,
        ...
        if config.CROP_MODE:
            if image_width <= 640 and image_height <= 640:
                ...
                crop_ratio = count_tiles(image_width, image_height, image_size=config.IMAGE_SIZE)
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        ...
        if config.IMAGE_SIZE == 1024 and config.BASE_SIZE == 1280:
        ...
        with torch.no_grad():
            for jdx in range(images_spatial_crop.size(0)):
                ...
                if torch.sum(patches).item() != 0:  # if all values = 0, no crop
                    ...
                    if config.PRINT_NUM_VIS_TOKENS:
                        print('=====================')
